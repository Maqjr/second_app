import 'package:flutter/material.dart';

import '../models/model.dart';
class Transaction_list extends StatelessWidget {
  String image='https://cdn.wallpapersafari.com/17/32/1n6ckw.jpg';
  String url='data:image/jpeg;base64,';
final List<model> model1;
Transaction_list(this.model1);
  @override
  Widget build(BuildContext context) {
     return  Container(

      height: 800,
      child: ListView.builder(
        itemBuilder: (context, index) {
          return Card(

            elevation: 2,
            shape: RoundedRectangleBorder(
                side: BorderSide(color: Colors.grey.shade200, width: 0.5),
                borderRadius: BorderRadius.circular(80)),

            child: Container(

              height: 170,
              child: Row(

                  children: [
                    Expanded(

                      child: Container(

                          child: Icon(Icons.account_balance_rounded,size: 80,)),
                    ),
                    Expanded(
                      flex: 6,
                      child:  Container(
                          color: Colors.black,
                        alignment: Alignment.center,


                        // decoration: BoxDecoration(color: Colors.grey),
                        child: Card(
                          //  shape: RoundedRectangleBorder(
                          // side: BorderSide(color: Colors.grey.shade200, width: 0.5),
                          //      borderRadius: BorderRadius.circular(80)),


                          child: ListTile(
                            leading: CircleAvatar(
                              radius: 25,
                              backgroundImage: NetworkImage(url),
                            ),
                            title: Text(model1[index].title,style: TextStyle(fontSize: 25,fontWeight: FontWeight.bold),),
                            subtitle: Row(
                              children: [
                                Column(
                                  children: [
                                    Text('Defence Phase 7 Dha Seher',style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: Colors.black),),
                                    Text(model1[index].date.toString(),style: TextStyle(fontSize: 15,fontStyle: FontStyle.italic),),
                                    Text('Aur bhai kya chal raha hai',style: TextStyle(fontSize: 15,fontStyle: FontStyle.italic)),
                                    Container(
                                      child: Row(
                                        children: [
                                          Container(
                                            margin: EdgeInsets.all(6),
                                            child: Column(
                                              children: [
                                                Container(
                                                    margin: EdgeInsets.only(bottom: 5),
                                                    child: Icon(Icons.bedroom_baby,color: Colors.blue,)),
                                                Text('bed: 2',style: TextStyle(color: Colors.black),)
                                              ],
                                            ),
                                          ),
                                          Container(
                                            margin: EdgeInsets.all(6),
                                            child: Column(
                                              children: [
                                                Container(
                                                    margin: EdgeInsets.only(bottom: 5),
                                                    child: Icon(Icons.bathroom,color: Colors.blue,)),
                                                Text('bath: 2',style: TextStyle(color: Colors.black))
                                              ],
                                            ),
                                          ),
                                          Container(
                                            margin: EdgeInsets.all(6),
                                            child: Column(
                                              children: [
                                                Container(
                                                    margin: EdgeInsets.only(bottom: 5),
                                                    child: Icon(Icons.crop_square_rounded,color: Colors.blue,)),
                                                Text('Sq:1800',style: TextStyle(color: Colors.black))
                                              ],
                                            ),
                                          )
                                        ],
                                      ),
                                    )

                                  ],
                                )

                              ],
                            ),

                            trailing: Container(

                              child: FloatingActionButton(onPressed:(){},
                                child: Container(


                                    child: Column(
                                      mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        Icon(Icons.add_box_sharp,size: 20,color: Colors.white,),
                                      ],
                                    )),

                              ),
                            ),),
                        ),
                      ),
                    ),

                    // Container(
                    //     padding: EdgeInsets.all(8),
                    //     margin: EdgeInsets.symmetric(vertical: 20,horizontal: 40),
                    //     decoration: BoxDecoration(border: Border.all(color: Colors.red,width: 2)),
                    //     child: (Text('\$ ${model1[index].amount.toString()}'))),
                    // Container(
                    //   padding: EdgeInsets.all(10),
                    //   child: Column(
                    //       children: [
                    //         Container(
                    //             margin: EdgeInsets.all(10),
                    //             child: Text(model1[index].title)),
                    //         Text(model1[index].date.toString()),
                    //         Text(model1[index].id)
                    //       ]
                    //   ),
                    // )

                  ]
              ),
            ),
          );
        } ,
        itemCount: model1.length,


      ),
    );
  }
}
